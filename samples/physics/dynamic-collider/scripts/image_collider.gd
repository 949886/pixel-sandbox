...
func _ready() -> void:
	if image == null:
		return
	var collider: StaticBody2D = generate_collider(image)
	add_child(collider)
...
static func generate_collider(image: Image, centered: bool = true) -> StaticBody2D:
	...
	var polys: Array[PackedVector2Array] = bitmap.opaque_to_polygons(Rect2(Vector2.ZERO, image.get_size()), 0.5)
	...
	for poly: PackedVector2Array in polys:
		...
		if centered:
			var half_size: Vector2 = Vector2(bitmap.get_size()) * 0.5
